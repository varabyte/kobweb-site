...
If you are hosting a static Kobweb site with a custom backend and finding that clean URLs are not working out of the box
(e.g., stuff like `about/` and `about.html` is fine but `about` causes a 404), you will need to set it up explicitly.

Some of our users have shared their configurations with us. If you have your own that you would like to
