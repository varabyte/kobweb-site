...
Sometimes, you may want certain parts of your layout to update based on the state of a page. For example, a commerce
site may have a header showing the number of items currently in the cart, which would need to be updated when the user
clicks the "Add to cart" button. Since `PageContext` provides a read-only view of the page data, your layout should
use a mutable data holder:

```kotlin
// jsMain/kotlin/com/mysite/components/layouts/PageLayout.kt

class StoreLayoutData(initialCartCount: Int) {
    var cartCount by mutableStateOf(initialCartCount)
}

@InitRoute
fun initStoreLayout(ctx: InitRouteContext) {
    ctx.data.add(StoreLayoutData(0))
}

@Layout
@Composable
fun StoreLayout(ctx: PageContext, content: @Composable () -> Unit) { /*...*/ }
```

```kotlin
// jsMain/kotlin/com/mysite/pages/Item.kt

@Page
@Layout(".components.layouts.StoreLayout")
@Composable
fun ItemPage(ctx: PageContext) {
    Button(onClick = { ctx.data.getValue<StoreLayoutData>().cartCount++ }) { Text("Add to cart") }
}
```
